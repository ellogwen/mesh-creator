...
class_name MeshCreator_Gizmos_VertexTranslateGizmoTool
...
func set_active() -> void:	
	var selectedVertices = _get_selected_vertices()
	_startPosition = Vector3.ZERO
	_currentPosition = Vector3.ZERO	
	var spatial: Spatial = _gizmoController.get_gizmo().get_spatial_node()
	var lastVertex = selectedVertices.back()
	if spatial != null and lastVertex != null:		
		_startPosition = lastVertex.get_position()
...
func _get_selected_vertices():
	return _gizmoController.get_gizmo().get_spatial_node().get_mc_mesh().get_vertices_selection(
		_gizmoController.get_gizmo().get_vertex_selection_store().get_store()
	)
	
func _get_handle_draw_position(handleIdx):
	match(handleIdx):
		0: return _currentPosition + (_axisRight * 0.1)
		1: return _currentPosition + (_axisUp * 0.1)
		2: return _currentPosition + (_axisForward * 0.1)