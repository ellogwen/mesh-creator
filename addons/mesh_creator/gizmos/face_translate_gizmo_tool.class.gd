...
func set_active() -> void:	
	...
	if _useFaceLocalAxis:
		use_axis_from_mesh()	
	else:
		use_axis_from_global()
...
func use_axis_from_mesh():
	var spatial: Spatial = _gizmoController.get_gizmo().get_spatial_node()
	if (spatial != null):		
		_axisRight = spatial.global_transform.basis.x
		_axisUp = spatial.global_transform.basis.y
		_axisForward = spatial.global_transform.basis.z
		var selectedFaces = _get_selected_faces()
...
func on_gizmo_add_handles(nextIndex: int) -> int:
	...
	if not selectedFaces.empty():	
		gizmo.add_handles(PoolVector3Array([spatial.to_local(_get_handle_draw_position(0))]), gizmo.get_plugin().HandleRightMaterial, false, false)	
		gizmo.add_handles(PoolVector3Array([spatial.to_local(_get_handle_draw_position(1))]), gizmo.get_plugin().HandleUpMaterial, false, false)	
		gizmo.add_handles(PoolVector3Array([spatial.to_local(_get_handle_draw_position(2))]), gizmo.get_plugin().HandleForwardMaterial, false, false)	
...
func _get_handle_draw_position(handleIdx):
	var spatial = _gizmoController.get_gizmo().get_spatial_node()
	match(handleIdx):
		0: return _currentGlobalPosition + (_axisRight * 0.25)
		1: return _currentGlobalPosition + (_axisUp * 0.25)
		2: return _currentGlobalPosition + (_axisForward * 0.25)

func _update_axis_indicators():
	MeshCreator_Indicator.add_line_global("face_translate_axis_x", _currentGlobalPosition, _currentGlobalPosition + _axisRight * 10, Color.red)	
	MeshCreator_Indicator.add_line_global("face_translate_axis_y", _currentGlobalPosition, _currentGlobalPosition + _axisUp * 10, Color.green)	
	MeshCreator_Indicator.add_line_global("face_translate_axis_z", _currentGlobalPosition, _currentGlobalPosition + _axisForward * 10, Color.blue)	
